...
window.checkValidJSON = function () {
  $.ajax({
    ...
    if (data === 'Valid') {
      ...
      $('#addDocumentForm').trigger('submit');
...
function renderProp(input) {
  ...
  if (
    ...
    return `<img src="${htmlEntities.encode(input)}" style="max-height:100%; max-width:100%; min-height:400px; min-width:400px"/>`;
  }
